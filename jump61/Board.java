...
abstract class Board {
    ...
    boolean isLegal(Color player, int r, int c) {
        if (isLegal(player) && exists(r, c)
            && color(r, c) != player.opposite()) {
            return true;
        }
        return false;
    }
    ...
    public String toString() {
        ...
        for (int r = 1; r <= size(); r += 1) {
            ...
            for (int c = 1; c <= size(); c += 1) {
                text += " ";
                if (spots(r, c) > 0) {
                    text += Integer.toString(spots(r, c));
                    if (color(r, c) == RED) {
                        text += "r";
                    } else {
                        text += "b";
    ...
    public String toDisplayString() {
        ...
        out.append(String.format(
            "Board of size %d at move %d", size(), numMoves()));
        return out.toString();
    }
    ...
    /** Returns the history of the board only if it is mutable. */
    abstract ArrayList<Board> getHistory();
