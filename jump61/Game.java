...
class Game {
    ...
    Game(Reader input, Writer prompts, Writer output, Writer errorOutput) {
        ...
        _playing = false;
        _playerRED = new HumanPlayer(this, RED);
        _playerBLUE = new AI(this, BLUE);
        _shouldQuit = false;
    }
    ...
    int play() {
        ...
        _out.flush();
        while (true) {
            if (_shouldQuit || !_inp.hasNextLine()) {
                break;
            }
            if (_playing) {
    ...
    private void setManual(Color player) {
        if (player == RED) {
            ...
            _playerBLUE = new HumanPlayer(this, BLUE);
        }
    }
    ...
    private void executeCommand(String cmnd) {
        ...
        default:
            if (cmnd.matches("\\d+") && _playing) {
                try {
                    int row = Integer.parseInt(cmnd);
                    int column = Integer.parseInt(_inp.next());
                    saveMove(row, column);
    ...
    /** Carries out the "manual" command. */
    private void makeManual() {
        try {
            Color p = readColor();
            _playing = false;
            setManual(p);
    ...
    private void doMove() {
        try {
            int m = Integer.parseInt(_inp.next());
            setMoveNumber(m);
            _inp.nextLine();
        } catch (GameException | NumberFormatException e) {
            reportError("Invalid Argument.");
    ...
    /** Carries out the "size" command. */
    private void doSize() {
        try {
            int n = Integer.parseInt(_inp.next());
            setSize(n);
            _inp.nextLine();
        } catch (GameException | NumberFormatException e) {
            reportError("Invalid Argument.");
    ...
    private void doSet() {
        try {
            ...
            int s = Integer.parseInt(_inp.next());
            String t = _inp.next();
            String p = "white";
            if (t.equals("R") || t.equals("r")) {
                p = "red";
            } else if (t.equals("B") || t.equals("b")) {
                p = "blue";
            }
            setSpots(r, c, s, p);
            _inp.nextLine();
    ...
    /** Carries out the "seed" command. */
    private void doSeed() {
        try {
            long seed = (long) Integer.parseInt(_inp.next());
            setSeed(seed);
            _inp.nextLine();
        } catch (GameException | NumberFormatException e) {
            reportError("Invalid Argument.");
